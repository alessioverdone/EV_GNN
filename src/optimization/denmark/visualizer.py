...
class RouteVisualizer:
    ...
    def _get_avg_availability(route_planner, site_id: str) -> float:
        if route_planner and hasattr(route_planner, 'availability_data'):
            if site_id in route_planner.availability_data:
                ...
                if values:
                    return np.mean(values)
        return float(__import__('random').randint(0, 5))
    ...
    def plot_route_comparison(graph: nx.DiGraph,
        ...
        used_station_lats, used_station_lons, used_station_names = [], [], []
            ...
            if node in graph.nodes:
                ...
                if route_planner:
                    ...
                    if stations:
                        ...
                        station_info = (f"USED CHARGING STATION<br>{station.name}<br>"
        ...
        if used_station_lats:
            fig.add_trace(go.Scattermapbox(
                ...
                marker={'size': 20, 'color': '#FFD700', 'symbol': 'circle',
                        'allowoverlap': True},
        ...
        other_station_lats, other_station_lons, other_station_names = [], [], []

        for station in list(ev_stations.values())[:100]:
            if station.src_node and station.src_node in graph:
                ...
                other_station_names.append(
                    f"Available Charging Station<br>{station.name}<br>"
                    f"ID: {station.station_id}<br>"
                    f"Power: {station.charging_power_kw} kW<br>"
                    f"Avg Availability: {avg_avail:.1f} chargers<br>"
                    f"Avg Speed: {avg_speed:.1f} km/h"
        ...
        if other_station_lats:
            fig.add_trace(go.Scattermapbox(
                ...
                marker={'size': 8, 'color': 'lightblue', 'symbol': 'circle', 'opacity': 0.6},
        ...
        center_lat = sum(all_lats) / len(all_lats) if all_lats else 56.15
        center_lon = sum(all_lons) / len(all_lons) if all_lons else 10.21

        fig.update_layout(
            mapbox=dict(style='open-street-map', center=dict(lat=center_lat, lon=center_lon), zoom=11),
            height=800,
            title={'text': "EV Route Comparison with Charging Stations", 'x': 0.5,
                   'xanchor': 'center', 'font': {'size': 18}},
            showlegend=True,
    ...
    @staticmethod
    def create_route_summary(graph: nx.DiGraph, path: List[str],
                             objective: PathObjectives, route_planner,
    ...
    def plot_network(graph, stations: Dict = None, title: str = "Road Network"):
        ...
        fig.add_trace(go.Scattermapbox(
            mode='lines', lon=edge_lons, lat=edge_lats,
            line={'width': 1, 'color': 'gray'}, name='Roads', hoverinfo='skip'
        ))
        ...
        fig.add_trace(go.Scattermapbox(
            mode='markers', lon=node_lons, lat=node_lats,
            marker={'size': 4, 'color': 'blue'}, text=node_texts,
        ...
        if stations:
            ...
            if station_lats:
                fig.add_trace(go.Scattermapbox(
                    mode='markers', lon=station_lons, lat=station_lats,
                    marker={'size': 8, 'color': 'green', 'symbol': 'circle'},
                    text=station_texts, name=f'Stations ({len(station_lats)})'
                ))

        center_lat = np.mean(node_lats) if node_lats else 56.15
        center_lon = np.mean(node_lons) if node_lons else 10.21
    ...
    def plot_parallel_coordinates(objectives: List[PathObjectives],
        ...
        df = pd.DataFrame({
            ...
            'Charging Stops': [float(o.num_charging_stops) for o in objectives],
    ...
    def plot_route_networkx(graph: nx.DiGraph, routes: List[List],
        ...
        nx.draw_networkx_edges(G2, pos=pos, ax=ax, edge_color='lightgrey',
                               arrows=True, width=1, alpha=0.5)
        ...
        for i, route in enumerate(routes):
            ...
            nx.draw_networkx_nodes(G2, pos=pos, ax=ax, nodelist=valid_nodes,
                                   node_color=colours[i % len(colours)], node_size=25, alpha=0.8)

        if charging_nodes:
            valid_charging = [n for n in charging_nodes if n in pos]
            charging_pos = {n: pos[n] for n in valid_charging}
            xs = [charging_pos[n][0] for n in valid_charging]
            ys = [charging_pos[n][1] for n in valid_charging]
            ax.scatter(xs, ys, marker='*', s=300, c='gold', edgecolors='darkorange',
                       linewidths=1.5, zorder=5, label=f'Charging Stops ({len(valid_charging)})')

        for special, colour, size in [(start_node, 'lime', 150), (end_node, 'red', 150)]:
            if special in pos:
                nx.draw_networkx_nodes(G2, pos=pos, ax=ax, nodelist=[special],
                                       node_color=colour, node_size=size)
        ...
        for i, route in enumerate(routes):
            ...
            legend_items.append(
                plt.Line2D([0], [0], marker='o', color='w',
                           markerfacecolor=colours[i % len(colours)], markersize=10, label=label)
            )
        legend_items += [
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='lime',
                       markersize=12, label=f'Start ({start_node})'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red',
                       markersize=12, label=f'End ({end_node})'),
        ]
        if charging_nodes:
            legend_items.append(
        ...
        ax.set_title(f'Route Comparison: {start_node} -> {end_node}', fontsize=14)
