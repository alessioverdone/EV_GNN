...
class EVRoutePlanner:
    ...
    def get_charging_stops(self, path: List, departure_time: datetime = None) -> List:
        ...
        for i in range(len(path) - 1):
            ...
            travel_hours = dist / speed if speed > 0 else dist / 50.0
        ...
        for node in self.node_to_stations.keys():
            try:
                ...
                if dist <= direct_dist * max_detour:
                    ...
                    if obj.total_time_min < best_score:
                        best_score = obj.total_time_min
                        best_path = full
    ...
    def _get_speed(self, site_id: str, current_time: datetime) -> float:
        ...
        speed_kmh = site_speeds[nearest_ts]
        return speed_kmh if speed_kmh >= 5.0 else 50.0
    ...
    def _get_availability(self, site_id: str, current_time: datetime) -> float:
        ...
        site_avail = self.availability_data.get(site_id, {})
        if not site_avail:
            return float(random.randint(0, 5))
