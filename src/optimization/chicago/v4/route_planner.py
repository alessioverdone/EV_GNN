...
class EVRoutePlanner:
    ...
    def get_energy_consumption(self, path: List) -> float:
        ...
        for i in range(len(path) - 1):
            if self.graph.has_edge(path[i], path[i + 1]):
                raw = self.graph.edges[path[i], path[i + 1]].get('distance_km', 0)
                total += self._to_km(raw) * self.ev_config.consumption_kwh_per_km
        return total
    ...
    def get_charging_stops(self, path: List, departure_time: datetime = None) -> List:
        ...
        for i in range(len(path) - 1):
            ...
            dist = self._to_km(edge.get('distance_km', 0))
    ...
    def find_path(self, start, end, max_detour: float = 1.3) -> List:
        ...
        for node in self.node_to_stations.keys():
            try:
                ...
                dist = sum(
                    self._to_km(self.graph.edges[full[i], full[i + 1]]['distance_km'])
    ...
    @staticmethod
    def _to_km(raw: float) -> float:
        return raw * 1.60934 if raw <= 5.0 else raw / 1000.0
