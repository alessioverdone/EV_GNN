...
def show_predictions(history_all,
    ...
    for s in range(ns):
        ...
        plot_sample_multifeature(history_all,
                                 ...
                                 n_idx,
                                 run_params,
                                 out_file=os.path.join(run_params.output_path, f'out_img_{s}.png'),
                                 sample_id=s + 1)
...
def plot_sample_multifeature(history_BN_lf,
    ...
    for f in range(F):
        ...
        ax = axes[f]
        past_real = history_BN_lf[b_idx, n_idx, :, f]  # (lags,)
        past_real = past_real * (max_val - min_val) + min_val
        ax.plot(x_past, past_real, linewidth=1.5, label="past")

        targ_real = target_BNTF[b_idx, n_idx, :, f]
        pred_real = pred_BNTF[b_idx, n_idx, :, f]

        targ_real = targ_real * (max_val - min_val) + min_val
        pred_real = pred_real * (max_val - min_val) + min_val
