...
def save_predictions(history_all,
    ...
    # ESPORTA CSV concatenati (uno per canale)
    if run_params.export_csv and preds_all is not None:
        export_inputs_csv(
            x_hist_cpu=history_all,
        ...
        export_predictions_csv(
            ...
            csv_dir=run_params.csv_out_path,
            start_time_str=(datetime.fromisoformat(run_params.start_time_test) + timedelta(
                minutes=run_params.lags * run_params.traffic_resolution)).isoformat(),
            # es. "2025-07-17 21:08:00"
            step_minutes=float(run_params.traffic_resolution),  # es. 5.0
            run_params=run_params,
            data_type='targets')
...
def generate_outputs_of_trained_model(model, device, test_loader, run_params):
...
def load_trained_model(model, device, args):
    model.to(device)
    model.eval()
...
def main():
    ...
    parser.add_argument("--show", action='store_true', help="Show sample plots (one window per sample)")
    parser.add_argument("--num_samples_to_show", type=int, default=3,
    ...
    model = get_model(run_params)
    device = torch.device(
        'cuda' if torch.cuda.is_available() and getattr(run_params, "accelerator", None) in ['gpu', 'cuda'] else 'cpu')
    ...
    run_params.output_path = os.path.join(run_params.output_path, run_params.dataset_name)
    run_params.csv_out_path = os.path.join(run_params.output_path, 'csv_out')
    os.makedirs(run_params.output_path, exist_ok=True)
    os.makedirs(run_params.csv_out_path, exist_ok=True)
...
if __name__ == "__main__":
    main()
>>>>>>> 50dc9fda781f8e9df4fc6cce2a2ff266f6b1e2a0:src/run_inference.py
