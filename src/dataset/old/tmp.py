"""
PEMS07 Graph Visualizer — Coordinate Reali + Analisi Componenti Connesse
=========================================================================
...
File richiesti nella stessa directory:
  ...
  • PEMS07.csv    → archi del grafo PEMS07 (from, to, cost)

Il mapping nodo_indice → coordinate GPS avviene tramite la colonna ID2 di ca_meta:
  I 883 sensori di PEMS07 corrispondono a ID2 nell'intervallo [3500, 4382].
  graph_idx = ID2 - 3500

Uso:
    python visualize_pems07.py

Output:
    pems07_graph_map.html
...
FILE_EDGES   = os.path.join(SCRIPT_DIR, "PEMS07.csv")
OUTPUT_HTML  = os.path.join(SCRIPT_DIR, "pems07_graph_map.html")

# Offset di ID2 in ca_meta corrispondente ai 883 sensori di PEMS07
PEMS07_ID2_OFFSET = 3500
PEMS07_N_NODES    = 883

# ── Palette colori per componenti connesse (20 colori distinti) ───────────────
CC_PALETTE = [
...
def print_component_report(G: nx.Graph, stats: dict, meta_df: pd.DataFrame) -> None:
    print()
    print("═" * 65)
    print("  ANALISI COMPONENTI CONNESSE — PEMS07")
    print("═" * 65)
    ...
    if stats["is_connected"]:
        ...
        for i, cc in enumerate(stats["components"]):
            ...
            size   = len(cc)
            pct    = size / total * 100
            # Recupera le autostrade principali dalla componente
            cc_meta = meta_df[meta_df["graph_idx"].isin(cc)]
            if not cc_meta.empty:
                fwys = cc_meta["Fwy"].value_counts().head(3)
                fwy_str = ", ".join([f"{f}({c})" for f, c in fwys.items()])
            else:
                fwy_str = "N/A"
...
def build_map(G: nx.Graph,
    ...
    for u, v, d in G.edges(data=True):
        ...
        folium.PolyLine(
            ...
            tooltip=f"Arco {u}↔{v} | dist: {d['weight']:.3f}",
        ).add_to(edge_group)
    edge_group.add_to(m)
    ...
    for node_idx in sorted(G.nodes()):
        ...
        radius    = 3.5 + 4.5 * (deg / max_deg)

        row = meta_idx.loc[node_idx] if node_idx in meta_idx.index else {}
        fwy      = row.get("Fwy",      "N/A") if isinstance(row, dict) else getattr(row, "Fwy",      "N/A")
        county   = row.get("County",   "N/A") if isinstance(row, dict) else getattr(row, "County",   "N/A")
        district = row.get("District", "N/A") if isinstance(row, dict) else getattr(row, "District", "N/A")
        sensor_id= row.get("ID",       "N/A") if isinstance(row, dict) else getattr(row, "ID",       "N/A")
        direction= row.get("Direction","N/A") if isinstance(row, dict) else getattr(row, "Direction","N/A")

        popup_html = (
            f"<b>Sensore PeMS #{sensor_id}</b><br>"
        ...
        folium.CircleMarker(
            ...
            tooltip=f"#{sensor_id} · {fwy} · grado {deg} · CC{cc_idx}",
    ...
    <div style="position:fixed;top:14px;left:50%;transform:translateX(-50%);
      ...
      <b style="font-size:15px;color:#00ffcc;">PEMS07 — Traffic Sensor Graph</b><br>
      <span style="font-size:11px;opacity:.75;">
        883 sensori · Los Angeles County · Calif. · Mag–Ago 2017
      </span><br>
...
def main():
    ...
    # ── 2. Carica ca_meta e filtra i 883 sensori di PEMS07 ────────────────
    print("[1/5] Carico ca_meta.csv e filtro sensori PEMS07 …")
    ...
    if missing_cols:
        ...
        print(f"      Colonne trovate: {meta_all.columns.tolist()}")
        sys.exit(1)

    meta_df = meta_all[
        meta_all["ID2"].between(PEMS07_ID2_OFFSET, PEMS07_ID2_OFFSET + PEMS07_N_NODES - 1)
    ].copy()
    meta_df["graph_idx"] = (meta_df["ID2"] - PEMS07_ID2_OFFSET).astype(int)

    print(f"      {len(meta_df)} sensori PEMS07 trovati in ca_meta")
    print(f"      Districts: {dict(meta_df['District'].value_counts())}")
    print(f"      County top3: {dict(meta_df['County'].value_counts().head(3))}")

    # Dizionario graph_idx → (lat, lon)
    coords = {
        int(row.graph_idx): (float(row.Lat), float(row.Lng))
        for row in meta_df.itertuples()
    }

    # ── 3. Carica archi e costruisce grafo ─────────────────────────────────
    print("[2/5] Carico archi e costruisco il grafo …")
    edges_df = pd.read_csv(FILE_EDGES)
    # Normalizza nomi colonne (gestisce 'cost', 'weight', 'distance')
